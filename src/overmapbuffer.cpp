    ...
    const tripoint_abs_omt &src, const tripoint_abs_omt &dest, path_type &ptype )
{
    ...
    [&]( const pf::node<point_rel_omt> &cur, const pf::node<point_rel_omt> * ) {
        ...
        res += travel_cost;
        res += trig_dist( finish, cur.pos );

        return res;
    };
    pf::path<point_rel_omt> route = ptype.only_road ?
                                    pf::find_path_4dir( start, finish, 2 * O, estimate ) :
                                    pf::find_path_8dir( start, finish, 2 * O, estimate );
                                  ...
                                  int radius, bool road_only )
{
    ...
    const auto path = pf::find_path_4dir( start, finish, 2 * O, estimate );
